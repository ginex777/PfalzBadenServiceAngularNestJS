...
export class BenutzerverwaltungComponent implements OnInit {
  ...
  anlegen(): void {
    ...
    if (!p.email || !p.password) { this.toast.error('E-Mail und Passwort sind Pflichtfelder.'); return; }
    this.anlegenLaedt.set(true);
    this.einstellungen.userAnlegen({ email: p.email, password: p.password, rolle: p.rolle as 'admin' | 'readonly' | 'mitarbeiter', vorname: p.vorname, nachname: p.nachname }).subscribe({
      next: user => {
  ...
  bearbeitenSpeichern(): void {
    ...
    if (!u) return;
    this.einstellungen.userAktualisieren(u.id, this.bearbeitenForm()).subscribe({
      next: updated => {
  ...
  loeschen(u: UserEintrag): void {
    ...
    this.einstellungen.userLoeschen(u.id).subscribe({
      next: () => {
        this.users.update(l => l.filter(x => x.id !== u.id));
        this._toast('✓ Benutzer gelöscht');
      },
      error: (e: { error?: { message?: string } }) => this.toast.error(e?.error?.message ?? 'Fehler beim Löschen.'),
    });
  }
