...
export class EuerFacade {
  ...
  readonly ergebnis = computed<EuerErgebnis>(() => {
    ...
    const alleZeilen: { typ: string; brutto: number; mwst: number; abzug: number; kategorie?: string }[] = [];
    Object.values(daten).forEach(monat => {
      (monat.inc || []).forEach(r => alleZeilen.push({ typ: 'inc', brutto: r.brutto, mwst: r.mwst ?? 19, abzug: r.abzug ?? 100, kategorie: r.kategorie }));
      (monat.exp || []).forEach(r => alleZeilen.push({ typ: 'exp', brutto: r.brutto, mwst: r.mwst ?? 19, abzug: r.abzug ?? 100, kategorie: r.kategorie }));
    });

    let inc0 = 0, inc7 = 0, inc19 = 0, ust7 = 0, ust19 = 0, vstGesamt = 0;
    const ausgabenByZeile: Record<string, number> = {};
    EUER_AUSGABEN_ZEILEN.forEach(z => { ausgabenByZeile[z.zeile] = 0; });

    alleZeilen.filter(r => r.typ === 'inc').forEach(r => {
      const netto = r.brutto / (1 + r.mwst / 100);
      const ust = r.brutto - netto;
      if (r.mwst === 0) inc0 += netto;
      else if (r.mwst === 7) { inc7 += netto; ust7 += ust; }
      else { inc19 += netto; ust19 += ust; }
    });

    alleZeilen.filter(r => r.typ === 'exp').forEach(r => {
      const netto = r.brutto / (1 + r.mwst / 100);
      const ust = r.brutto - netto;
    ...
    return {
      ...
      ausgaben: {
        zeilen: EUER_AUSGABEN_ZEILEN.map(z => ({ zeile: z.zeile, label: z.label, betrag: ausgabenByZeile[z.zeile] || 0 })),
        summe: sumExp,
      },
