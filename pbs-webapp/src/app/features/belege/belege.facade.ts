...
export class BelegeFacade {
  ...
  readonly verfuegbareJahre = computed(() => {
    const years = [
      ...new Set(this.belege().map((b) => new Date(b.erstellt_am ?? '').getFullYear())),
    ]
      .filter((y) => !isNaN(y))
      .sort()
      .reverse();
  ...
  readonly spaetesteAufbewahrung = computed(() => {
    const dates = this.belege()
      .map((b) => b.aufbewahrung_bis)
      .filter(Boolean)
      .sort() as string[];
  ...
  readonly gefilterteBelege = computed(() => {
    ...
    return this.belege()
      ...
      .filter(
        (b) =>
          !q ||
          [b.filename, b.notiz, b.buchung_name, b.buchung_kategorie]
            .join(' ')
            .toLowerCase()
            .includes(q),
      )
  ...
  ladeDaten(): void {
    ...
    this.service.alleLaden(this.aktivesJahr()).subscribe({
      ...
      error: () => {
        this.toast.error('Belege konnten nicht geladen werden.');
        this.laedt.set(false);
      },
    });
  }
  ...
  loeschenAusfuehren(): void {
    ...
    this.service.loeschen(id).subscribe({
      next: () => {
        this.belege.update((list) => list.filter((b) => b.id !== id));
        this.loeschKandidat.set(null);
      },
      error: () => {
        this.toast.error('Beleg konnte nicht gelöscht werden.');
        this.loeschKandidat.set(null);
      },
    });
  }
  ...
  downloadUrl(id: number, inline = false): string {
    return this.service.downloadUrl(id, inline);
  }
  dateigroesseFormatieren(bytes: number): string {
    return dateigroesseFormatieren(bytes);
  }
  filterSetzen(filter: BelegeFilter): void {
    this.aktiverFilter.set(filter);
    this.aktuelleSeite.set(1);
  }
  jahrFilterSetzen(jahr: number | null): void {
    this.jahrFilter.set(jahr);
    this.aktuelleSeite.set(1);
  }
  ...
  notizBearbeitungStarten(id: number): void {
    this.notizBearbeitungId.set(id);
  }
  notizBearbeitungAbbrechen(): void {
    this.notizBearbeitungId.set(null);
  }
  ...
  viewerOeffnen(beleg: Beleg): void {
    this.viewerBeleg.set(beleg);
  }
  viewerSchliessen(): void {
    this.viewerBeleg.set(null);
  }
