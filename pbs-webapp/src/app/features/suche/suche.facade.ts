...
export class SucheFacade {
  ...
  readonly daten = signal<SucheErgebnis>({
    rechnungen: [],
    angebote: [],
    kunden: [],
    hausmeister: [],
  ...
  suchbegriffSetzen(q: string): void {
    this.suchbegriff.set(q);
  }

  zuRechnung(id: number): void {
    this.router.navigate(['/rechnungen'], { queryParams: { id } });
  }
  zuAngebot(id: number): void {
    this.router.navigate(['/angebote'], { queryParams: { id } });
  }
