...
export class AuditLogFacade {
  ...
  readonly gefilterteEintraege = computed(() => {
    ...
    return this.eintraege()
      ...
      .filter(e => !q || [e.tabelle, e.aktion, e.neu_wert, e.alt_wert].join(' ').toLowerCase().includes(q))
  ...
  filterSetzen(aktion: AuditAktion): void { this.aktiverAktionFilter.set(aktion); this.aktuelleSeite.set(1); }
  tabelleFilterSetzen(tabelle: string): void { this.aktiveTabelleFilter.set(tabelle); this.aktuelleSeite.set(1); }
  suchbegriffAktualisieren(q: string): void { this.suchbegriff.set(q); this.aktuelleSeite.set(1); }
  seiteVor(): void { this.aktuelleSeite.update(s => Math.min(s + 1, this.gesamtSeiten())); }
  seiteZurueck(): void { this.aktuelleSeite.update(s => Math.max(s - 1, 1)); }
}
