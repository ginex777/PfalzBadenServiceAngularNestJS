...
import {
  ...
  OffenePostenRechnung, LEERES_FORMULAR,
} from './kunden.models';
...
export class KundenFacade {
  ...
  speichern(daten: KundenFormularDaten): void {
    ...
    const anfrage = editId
      ? this.service.kundeAktualisieren(editId, daten)
      : this.service.kundeErstellen(daten);
  ...
  loeschenAusfuehren(): void {
    ...
    if (id === null) return;
    this.service.kundeLoeschen(id).subscribe({
      next: () => {
  ...
  offenePostenAnzeigen(kundeId: number): void {
    ...
    const kundenRe = this.cachedRechnungen.filter(r => r.kunden_id === kundeId || r.empf === kunde.name);
    ...
    const rechnungen: OffenePostenRechnung[] = kundenRe
      ...
      .map(r => ({
        id: r.id, nr: r.nr, titel: r.titel ?? '', datum: r.datum,
        brutto: r.brutto, bezahlt: r.bezahlt,
        ueberfaellig: !r.bezahlt && !!r.frist && new Date(r.frist) < heute,
        frist: r.frist, bezahlt_am: r.bezahlt_am,
      }));

    this.offenePosten.set({
      kundeId, kundeName: kunde.name,
  ...
  sammelMahnungOeffnen(kundeId: number): void {
    ...
    if (!kunde) return;
    const heute = new Date(); heute.setHours(0, 0, 0, 0);
    const ueberfaellig = this.cachedRechnungen.filter(r =>
      (r.kunden_id === kundeId || r.empf === kunde.name) && !r.bezahlt && !!r.frist && new Date(r.frist) < heute
    );
    const gesamtBrutto = ueberfaellig.reduce((s, r) => s + (r.brutto ?? 0), 0);
    const reList = ueberfaellig.map(r => {
    ...
    this.sammelMahnungBetreff.set(`Zahlungserinnerung — ${ueberfaellig.length} offene Rechnung(en) — ${kunde.name}`);
