...
export class WiederkehrendeAusgabenFacade {
  ...
  speichern(): void {
    ...
    if (!daten.name || !daten.brutto) { this.fehler.set('Name und Betrag sind Pflichtfelder.'); return; }
    ...
    anfrage.subscribe({
      ...
      error: () => this.fehler.set('Ausgabe konnte nicht gespeichert werden.'),
    });
  }
  ...
  loeschenAusfuehren(): void {
    ...
    this.service.loeschen(id).subscribe({
      ...
      error: () => { this.fehler.set('Ausgabe konnte nicht gelöscht werden.'); this.loeschKandidat.set(null); },
    });
  }
