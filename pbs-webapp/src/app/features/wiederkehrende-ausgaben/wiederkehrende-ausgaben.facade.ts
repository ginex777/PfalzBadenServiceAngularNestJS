...
export class WiederkehrendeAusgabenFacade {
  ...
  readonly gefilterteAusgaben = computed(() => {
    ...
    return this.ausgaben().filter(
      (a) => !q || a.name.toLowerCase().includes(q) || a.kategorie.toLowerCase().includes(q),
    );
  });
  ...
  readonly aktiveSumme = computed(() => {
    ...
    const vst = aktive.reduce(
      (s, a) => s + (a.brutto - a.brutto / (1 + a.mwst / 100)) * (a.abzug / 100),
      0,
    );
  ...
  formularSchliessen(): void {
    ...
    this.bearbeiteteAusgabe.set(null);
  }

  speichern(): void {
    const daten = this.formularDaten();
    if (!daten.name || !daten.brutto) {
      this.toast.error('Name und Betrag sind Pflichtfelder.');
      return;
    }
    ...
    anfrage.subscribe({
      next: (gespeichert) => {
        if (editId)
          this.ausgaben.update((list) => list.map((a) => (a.id === editId ? gespeichert : a)));
  ...
  aktivToggle(id: number, aktiv: boolean): void {
    ...
    this.service.aktualisieren(id, { ...a, aktiv }).subscribe({
      next: (aktualisiert) =>
        this.ausgaben.update((list) => list.map((x) => (x.id === id ? aktualisiert : x))),
    });
  }
  ...
  loeschenAusfuehren(): void {
    ...
    this.service.loeschen(id).subscribe({
      next: () => {
        this.ausgaben.update((list) => list.filter((a) => a.id !== id));
        this.loeschKandidat.set(null);
      },
      error: () => {
        this.toast.error('Ausgabe konnte nicht gelöscht werden.');
        this.loeschKandidat.set(null);
      },
    });
  }
  ...
  formularFeldAktualisieren<K extends keyof WiederkehrendeAusgabeFormularDaten>(
    feld: K,
    wert: WiederkehrendeAusgabeFormularDaten[K],
  ): void {
