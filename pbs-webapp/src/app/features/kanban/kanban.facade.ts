...
export class KanbanFacade {
  ...
  readonly gefilterteTasksProSpalte = computed(() => {
    ...
    const gefiltert = this.tasks().filter(t => {
      if (q && ![t.titel, t.beschreibung, t.bearbeiter, t.kategorie].join(' ').toLowerCase().includes(q)) return false;
    ...
    return {
      ...
      inprogress: gefiltert.filter(t => t.status === 'inprogress').sort((a, b) => a.position - b.position),
      done: gefiltert.filter(t => t.status === 'done').sort((a, b) => a.position - b.position),
      blocked: gefiltert.filter(t => t.status === 'blocked').sort((a, b) => a.position - b.position),
    };
  });

  readonly eindeutigeBearbeiter = computed(() =>
    [...new Set(this.tasks().map(t => t.bearbeiter).filter((b): b is string => !!b))].sort()
  );
  ...
  formularOeffnen(task?: Task, standardStatus: KanbanSpalte = 'todo'): void {
    ...
    this.formularDaten.set(task ? {
      titel: task.titel, beschreibung: task.beschreibung ?? '', datum: task.datum ?? '',
      bearbeiter: task.bearbeiter ?? '', kategorie: task.kategorie,
      prioritaet: task.prioritaet, status: task.status,
    } : { ...LEERES_TASK_FORMULAR, status: standardStatus });
    this.formularSichtbar.set(true);
  }

  formularSchliessen(): void { this.formularSichtbar.set(false); this.bearbeiteterTask.set(null); }
  ...
  speichern(): void {
    ...
    anfrage.subscribe({
      next: gespeichert => {
        if (editId) this.tasks.update(list => list.map(t => t.id === editId ? gespeichert : t));
  ...
  dragStart(id: number): void { this.dragTaskId.set(id); }
  dragEnd(): void { this.dragTaskId.set(null); }

  drop(neuerStatus: KanbanSpalte): void {
    const id = this.dragTaskId();
    if (!id) return;
    const task = this.tasks().find(t => t.id === id);
    if (!task || task.status === neuerStatus) { this.dragTaskId.set(null); return; }
  ...
  filterLeeren(): void { this.filter.set({ suchbegriff: '', kategorie: '', bearbeiter: '', prioritaet: '' }); }

  formularFeldAktualisieren<K extends keyof TaskFormularDaten>(feld: K, wert: TaskFormularDaten[K]): void {
