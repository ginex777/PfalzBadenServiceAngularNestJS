...
export class PdfArchivFacade {
  ...
  readonly gefilterteEintraege = computed(() => {
    ...
    return this.eintraege()
      ...
      .filter(e => !q || [e.referenz_nr, e.empf, e.titel, e.filename].join(' ').toLowerCase().includes(q))
  ...
  alleLoeschen(): void {
    if (!confirm('Alle PDF-Einträge aus dem Protokoll löschen? Die Dateien bleiben erhalten.')) return;
  ...
  filterSetzen(filter: PdfArchivFilter): void { this.aktiverFilter.set(filter); this.aktuelleSeite.set(1); }
