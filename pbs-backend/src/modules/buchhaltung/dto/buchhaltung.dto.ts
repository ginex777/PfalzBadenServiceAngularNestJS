import {
  IsString, IsOptional, IsNumber, IsDateString, IsArray,
  ValidateNested, IsIn, Min, Max, IsInt,
...
export class BuchhaltungEintragDto {
  ...
  @ApiPropertyOptional() @IsOptional() @IsNumber() @Min(0) @Max(100) mwst?: number;
  @ApiPropertyOptional() @IsOptional() @IsNumber() @Min(0) @Max(100) abzug?: number;
...
export class BatchSpeichernDto {
  @ApiProperty() @IsInt() @Min(2000) @Max(2100) jahr!: number;
  @ApiProperty() @IsInt() @Min(0) @Max(11) monat!: number;
  @ApiProperty({ type: [BuchhaltungEintragDto] }) @IsArray() @ValidateNested({ each: true }) @Type(() => BuchhaltungEintragDto) rows!: BuchhaltungEintragDto[];
}
