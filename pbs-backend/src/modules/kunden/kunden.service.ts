...
export class KundenService {
  ...
  async kundeErstellen(daten: Record<string, unknown>) {
    ...
    await this.audit.protokollieren('kunden', kunde.id, 'CREATE', null, kunde);
    return { ...kunde, id: Number(kunde.id) };
  }

  async kundeAktualisieren(id: number, daten: Record<string, unknown>) {
    ...
    await this.audit.protokollieren('kunden', BigInt(id), 'UPDATE', alt, neu);
    return { ...neu, id: Number(neu.id) };
  }

  async kundeLoeschen(id: number) {
    const alt = await this.prisma.kunden.findUnique({ where: { id: BigInt(id) } });
    if (!alt) throw new NotFoundException(`Kunde ${id} nicht gefunden`);
    ...
    await this.audit.protokollieren('kunden', BigInt(id), 'DELETE', alt, null);
    return { ok: true };
  }
}
