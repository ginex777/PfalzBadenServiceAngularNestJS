...
export class RechnungPdfGenerator {
  ...
  async erstellen(rechnungId: number): Promise<{ token: string; url: string }> {
    ...
    const zahlDatum = rechnung.datum ? this.render.addTage(rechnung.datum.toISOString().slice(0, 10), rechnung.zahlungsziel ?? 14) : '–';
    ...
    const html = this.render.templateRendern('rechnung.hbs', kontext);
