...
export class MitarbeiterPdfGenerator {
  ...
  async abrechnungPdfErstellen(mitarbeiterId: number): Promise<{ token: string; url: string }> {
    ...
    const stunden = stundenRows.map(s => ({
      ...
      datumFormatiert: this.render.datumFormatieren(s.datum.toISOString().slice(0, 10)),
    }));
    ...
    const kontext = {
      ...
      stundenlohnFormatiert: Number(ma.stundenlohn) > 0 ? this.render.fmtEur(Number(ma.stundenlohn)) : 'variabel',
      stunden, gesamtStunden, gesamtGrundlohn, gesamtZuschlag, gesamtLohn, bezahlt, offen,
      offerFarbe: offen > 0 ? 'red' : 'green',
    };

    const html = this.render.templateRendern('mitarbeiter-abrechnung.hbs', kontext);
